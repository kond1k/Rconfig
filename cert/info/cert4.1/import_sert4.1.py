...
def exec_command_get_cert(host, user, secret, port=22):
    ...
    try:
        ...
        time.sleep(2)
        data.send('mount -t nfs 10.252.253.245:/srv/nfs/shares/hosts_certs4.1/ -o rw,proto=tcp,soft,nolock /media \n')
        time.sleep(2)
...
if __name__ == '__main__':
    os.system("openssl pkcs7 -inform DER -outform PEM -in /srv/nfs/shares/hosts_certs4.1/certs.p7b -print_certs > /srv/nfs/shares/hosts_certs4.1/certificate_bundle.cer")
    with open("/srv/nfs/shares/hosts_certs4.1/certificate_bundle.cer") as f:
        a = f.read().split("\n\n")
        for cert in a:
            hostname = re.search(r'subject=.+CN=(\S+)', cert)
            if hostname:
                with open(f'/srv/nfs/shares/hosts_certs4.1/{hostname.group(1)}.cer','w') as fc:
                    fc.write(cert)
